...
ЗАДАНИЕ: "Слишком древний шифр":
  - Вы отправились в путешествие на необитаемый остров и конечно же в очередной вылазке
  ...
  - К вашему счастью рядом с менее успешными и уже неговорящими путешественниками
    находился папирус, где были написаны правила для решения этого "ребуса".
...
"""
iFirstNum = int( input( 'Введите число из первого поля: ', ) )
Password = []
iSumPair = 0
i1 = 0
i2 = 0
for i1 in range( 1, int( ( iFirstNum + 1 ) / 2 ) ) :
    for i2 in range( i1 + 1, iFirstNum ) :
        iSumPair = i1 + i2
        if ( iFirstNum % iSumPair ) == 0 :
            Password.append( i1 )
            Password.append( i2 )
CodToFree = ''
for iStr in  Password :
    CodToFree = CodToFree + str( iStr )
print( 'Код для освобождения : > ', CodToFree, ' <', sep='' )
