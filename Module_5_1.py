...
Также должен обладать методом go_to(new_floor), где
...
Пункты задачи:
  ...
  - Внутри метода __init__ создайте атрибуты объекта self.name
        и self.number_of_floors, присвойте им переданные значения.
  - Создайте метод go_to с параметром new_floor и напишите логику
...
Примечания:
  - self - это переменная указывающая на объект.
