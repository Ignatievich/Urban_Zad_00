...
def string_info( String ) : # Принимает строку
    count_calls()
    Stat_Var = ( len( String ), String.upper(), String.lower() )
    return Stat_Var

def is_contains( String, List_to_search ) : #
    count_calls()
    List_to_search_lower = [ item.lower() for item in List_to_search ]
    if String.lower() in List_to_search_lower : bYesNo = True
    else                                      : bYesNo = False
    return bYesNo
...
print(is_contains('noCycle', ['reCycling', 'Cyclic', 'Cycle']))
